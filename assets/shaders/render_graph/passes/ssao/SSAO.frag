...
#define RENDER_GRAPH_SSAO_KERNEL_SET 1
#include <shaders/render_graph/common/ParameterGroups.h>

layout(set = 2, binding = 0) uniform sampler2D tex_gbuffer_depth;
layout(set = 2, binding = 1) uniform sampler2D tex_gbuffer_normal;
layout(set = 2, binding = 2) uniform sampler2D tex_ssao_kernel_noise;
...
void main()
{
	...
	vec2 noise_uv = in_uv * textureSize(tex_gbuffer_depth, 0) / textureSize(tex_ssao_kernel_noise, 0).xy;
	vec3 random_direction = vec3(texture(tex_ssao_kernel_noise, noise_uv).xy, 0.0f);
	...
	float occlusion = getOcclusion(origin_vs, tbn, ssao_kernel.radius, 0.01f, ssao_kernel.num_samples);

	out_ssao = 1.0f - occlusion / ssao_kernel.num_samples;
	out_ssao = pow(out_ssao, ssao_kernel.intensity);
}
