...
namespace scapes::visual::shaders::spirv
{
	namespace parser
	{
		static bool getContents(const foundation::io::URI &uri, foundation::io::FileSystem *file_system, std::string &result)
		{
			size_t size = 0;
			...
			if (!data)
			{
				foundation::Log::error("parser::getContents(): can't get data from \"%s\"\n", uri.c_str());
				return false;
			}
		...
		static bool parseIncludes(const char *source, foundation::io::FileSystem *file_system, std::set<std::string> &includes)
		{
			const char *s = source;
	...
	namespace shaderc
	{
		...
		{
			SCAPES_PROFILER();
			foundation::io::FileSystem *file_system = reinterpret_cast<foundation::io::FileSystem *>(user_data);
			assert(file_system);
			...
			if (!data)
			{
				foundation::Log::error("shaderc::include_resolver(): can't load include at \"%s\"\n", target_path.c_str());
				return result;
			}
	...
	Compiler::Compiler(foundation::io::FileSystem *file_system)
		: file_system(file_system)
	{
		cache = new shaders::ShaderCache(ShaderILType::SPIRV);
	...
	)
	{
		...
		uint64_t hash = 0;
		{
			...
			{
				uint64_t mtime = file_system->mtime(foundation::io::URI(path.c_str()));
	...
	)
	{
		...
		{
			foundation::Log::error("Compiler::createShaderIL(): can't compile shader at \"%s\"\n", path);
			foundation::Log::error("\t%s\n", shaderc_result_get_error_message(compilation_result));
