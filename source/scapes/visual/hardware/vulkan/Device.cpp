...
#include <hardware/vulkan/DescriptorSetLayoutCache.h>
...
#include <hardware/vulkan/RenderPassBuilder.h>
...
namespace scapes::visual::hardware::vulkan
{
	...
	hardware::VertexBuffer Device::createVertexBuffer(
		BufferType type,
	...
	)
	{
		...
		return reinterpret_cast<hardware::VertexBuffer>(result);
	}

	hardware::IndexBuffer Device::createIndexBuffer(
		BufferType type,
	...
	)
	{
		...
		return reinterpret_cast<hardware::IndexBuffer>(result);
	}

	hardware::Texture Device::createTexture2D(
		uint32_t width,
		uint32_t height,
	...
	)
	{
		...
		return reinterpret_cast<hardware::Texture>(result);
	}

	hardware::Texture Device::createTexture2DMultisample(
		uint32_t width,
		uint32_t height,
		Format format,
		Multisample samples
	...
	hardware::Texture Device::createTexture2DArray(
		uint32_t width,
		uint32_t height,
		uint32_t num_mipmaps,
		uint32_t num_layers,
	...
	hardware::Texture Device::createTexture3D(
		uint32_t width,
		uint32_t height,
		uint32_t depth,
		uint32_t num_mipmaps,
	...
	hardware::Texture Device::createTextureCube(
		uint32_t size,
		uint32_t num_mipmaps,
		Format format,
		const void *data,
	...
	hardware::Texture Device::createTextureStorage(
		uint32_t width,
		uint32_t height,
		Format format
	)
	...
	hardware::FrameBuffer Device::createFrameBuffer(
	...
	)
	{
		...
		return reinterpret_cast<hardware::FrameBuffer>(result);
	}

	hardware::RenderPass Device::createRenderPass(
		uint32_t num_attachments,
	...
	)
	{
		...
		return reinterpret_cast<hardware::RenderPass>(result);
	}
	...
	hardware::CommandBuffer Device::createCommandBuffer(
	...
	)
	{
		...
		if (vkAllocateCommandBuffers(context->getDevice(), &info, &result->command_buffer) != VK_SUCCESS)
		{
			...
			destroyCommandBuffer(reinterpret_cast<hardware::CommandBuffer>(result));
			return nullptr;
		}
		...
		return reinterpret_cast<hardware::CommandBuffer>(result);
	}

	hardware::UniformBuffer Device::createUniformBuffer(
		BufferType type,
		uint32_t size,
		const void *data
	)
	{
		...
		return reinterpret_cast<hardware::UniformBuffer>(result);
	}

	hardware::Shader Device::createShaderFromSource(
		ShaderType type,
		uint32_t size,
		const char *data,
		const char *path
	...
	hardware::Shader Device::createShaderFromIL(
		ShaderType type,
	...
	)
	{
		...
		return reinterpret_cast<hardware::Shader>(result);
	}

	hardware::BindSet Device::createBindSet()
	{
		BindSet *result = new BindSet();
		memset(result, 0, sizeof(BindSet));

		return reinterpret_cast<hardware::BindSet>(result);
	}

	hardware::GraphicsPipeline Device::createGraphicsPipeline()
	{
		...
		return reinterpret_cast<hardware::GraphicsPipeline>(result);
	}

	hardware::BottomLevelAccelerationStructure Device::createBottomLevelAccelerationStructure(
	...
	)
	{
		...
		if (!Utils::createAccelerationStructure(context, type, build_flags, num_geometries, vk_geometries, max_primitives, result))
		{
			...
			destroyBottomLevelAccelerationStructure(reinterpret_cast<hardware::BottomLevelAccelerationStructure>(result));
			return SCAPES_NULL_HANDLE;
		}
		...
		return reinterpret_cast<hardware::BottomLevelAccelerationStructure>(result);
	}

	hardware::TopLevelAccelerationStructure Device::createTopLevelAccelerationStructure(
	...
	)
	{
		...
		{
			destroyTopLevelAccelerationStructure(reinterpret_cast<hardware::TopLevelAccelerationStructure>(result));
			return SCAPES_NULL_HANDLE;
		}
		...
		return reinterpret_cast<hardware::TopLevelAccelerationStructure>(result);
	}

	hardware::RayTracePipeline Device::createRayTracePipeline(
		// ???
	)
	{
		...
		return reinterpret_cast<hardware::RayTracePipeline>(result);
	}

	hardware::SwapChain Device::createSwapChain(void *native_window)
	{
		...
		if (result->surface == VK_NULL_HANDLE)
		{
			...
			destroySwapChain(reinterpret_cast<hardware::SwapChain>(result));
			return false;
		}
		...
		return reinterpret_cast<hardware::SwapChain>(result);
	}

	void Device::destroyVertexBuffer(hardware::VertexBuffer vertex_buffer)
	{
	...
	void Device::destroyIndexBuffer(hardware::IndexBuffer index_buffer)
	{
	...
	void Device::destroyTexture(hardware::Texture texture)
	{
	...
	void Device::destroyFrameBuffer(hardware::FrameBuffer frame_buffer)
	{
	...
	void Device::destroyRenderPass(hardware::RenderPass render_pass)
	{
	...
	void Device::destroyCommandBuffer(hardware::CommandBuffer command_buffer)
	{
	...
	void Device::destroyUniformBuffer(hardware::UniformBuffer uniform_buffer)
	{
	...
	void Device::destroyShader(hardware::Shader shader)
	{
	...
	void Device::destroyBindSet(hardware::BindSet bind_set)
	{
	...
	void Device::destroyGraphicsPipeline(hardware::GraphicsPipeline pipeline)
	{
	...
	void Device::destroyBottomLevelAccelerationStructure(hardware::BottomLevelAccelerationStructure acceleration_structure)
	{
	...
	void Device::destroyTopLevelAccelerationStructure(hardware::TopLevelAccelerationStructure acceleration_structure)
	{
		if (acceleration_structure == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::destroyRayTracePipeline(hardware::RayTracePipeline pipeline)
	{
		if (pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::destroySwapChain(hardware::SwapChain swap_chain)
	{
	...
	uint32_t Device::getNumSwapChainImages(hardware::SwapChain swap_chain)
	{
	...
	void Device::setTextureSamplerWrapMode(hardware::Texture texture, SamplerWrapMode mode)
	{
	...
	void Device::setTextureSamplerDepthCompare(hardware::Texture texture, bool enabled, DepthCompareFunc func)
	{
		assert(texture != SCAPES_NULL_HANDLE && "Invalid texture");
		Texture *vk_texture = reinterpret_cast<Texture *>(texture);
	...
	void Device::generateTexture2DMipmaps(hardware::Texture texture)
	{
		assert(texture != SCAPES_NULL_HANDLE && "Invalid texture");
	...
	/*
	 */
	void *Device::map(hardware::VertexBuffer vertex_buffer)
	{
	...
	void Device::unmap(hardware::VertexBuffer vertex_buffer)
	{
		assert(vertex_buffer != SCAPES_NULL_HANDLE && "Invalid buffer");
	...
	void *Device::map(hardware::IndexBuffer index_buffer)
	{
	...
	void Device::unmap(hardware::IndexBuffer index_buffer)
	{
	...
	void *Device::map(hardware::UniformBuffer uniform_buffer)
	{
	...
	void Device::unmap(hardware::UniformBuffer uniform_buffer)
	{
	...
	/*
	 */
	void Device::flush(hardware::BindSet bind_set)
	{
		if (bind_set == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::flush(hardware::GraphicsPipeline graphics_pipeline)
	{
		...
		for (uint32_t i = 0; i < vk_graphics_pipeline->num_bind_sets; ++i)
			flush(reinterpret_cast<hardware::BindSet>(vk_graphics_pipeline->bind_sets[i]));
	...
	void Device::flush(hardware::RayTracePipeline raytrace_pipeline)
	{
		...
		for (uint32_t i = 0; i < vk_raytrace_pipeline->num_bind_sets; ++i)
			flush(reinterpret_cast<hardware::BindSet>(vk_raytrace_pipeline->bind_sets[i]));
	...
	/*
	 */
	bool Device::acquire(hardware::SwapChain swap_chain, uint32_t *new_image)
	{
		if (swap_chain == SCAPES_NULL_HANDLE)
			return false;
	...
	bool Device::present(hardware::SwapChain swap_chain, uint32_t num_wait_command_buffers, const hardware::CommandBuffer *wait_command_buffers)
	{
		SwapChain *vk_swap_chain = reinterpret_cast<SwapChain *>(swap_chain);
	...
	void Device::clearBindSets(
		hardware::RayTracePipeline raytrace_pipeline
	)
	{
		if (raytrace_pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::setBindSet(hardware::RayTracePipeline raytrace_pipeline, uint8_t binding, hardware::BindSet bind_set)
	{
	...
	void Device::clearRaygenShaders(hardware::RayTracePipeline raytrace_pipeline)
	{
		if (raytrace_pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::addRaygenShader(hardware::RayTracePipeline raytrace_pipeline, hardware::Shader shader)
	{
		if (raytrace_pipeline == SCAPES_NULL_HANDLE)
			return;
		...
		Shader *vk_shader = reinterpret_cast<Shader *>(shader);
		assert(vk_shader->type == hardware::ShaderType::RAY_GENERATION);
	...
	void Device::clearHitGroupShaders(hardware::RayTracePipeline raytrace_pipeline)
	{
		if (raytrace_pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	)
	{
		...
		assert(vk_intersection_shader == nullptr || vk_intersection_shader->type == hardware::ShaderType::INTERSECTION);
		assert(vk_anyhit_shader == nullptr || vk_anyhit_shader->type == hardware::ShaderType::ANY_HIT);
		assert(vk_closesthit_shader == nullptr || vk_closesthit_shader->type == hardware::ShaderType::CLOSEST_HIT);
	...
	void Device::clearMissShaders(hardware::RayTracePipeline raytrace_pipeline)
	{
		if (raytrace_pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::addMissShader(hardware::RayTracePipeline raytrace_pipeline, hardware::Shader shader)
	{
		if (raytrace_pipeline == SCAPES_NULL_HANDLE)
			return;
		...
		Shader *vk_shader = reinterpret_cast<Shader *>(shader);
		assert(vk_shader->type == hardware::ShaderType::MISS);
	...
	/*
	 */
	void Device::clearPushConstants(hardware::GraphicsPipeline graphics_pipeline)
	{
	...
	void Device::setPushConstants(hardware::GraphicsPipeline graphics_pipeline, uint8_t size, const void *data)
	{
	...
	void Device::clearBindSets(hardware::GraphicsPipeline graphics_pipeline)
	{
		if (graphics_pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::setBindSet(hardware::GraphicsPipeline graphics_pipeline, uint8_t binding, hardware::BindSet bind_set)
	{
	...
	void Device::clearShaders(hardware::GraphicsPipeline graphics_pipeline)
	{
		if (graphics_pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::setShader(hardware::GraphicsPipeline graphics_pipeline, ShaderType type, hardware::Shader shader)
	{
		if (graphics_pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::clearVertexStreams(hardware::GraphicsPipeline graphics_pipeline)
	{
		if (graphics_pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::setVertexStream(hardware::GraphicsPipeline graphics_pipeline, uint8_t binding, hardware::VertexBuffer vertex_buffer)
	{
	...
	/*
	 */
	void Device::setViewport(hardware::GraphicsPipeline graphics_pipeline, int32_t x, int32_t y, uint32_t width, uint32_t height)
	{
		if (graphics_pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::setScissor(hardware::GraphicsPipeline graphics_pipeline, int32_t x, int32_t y, uint32_t width, uint32_t height)
	{
		if (graphics_pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::setPrimitiveType(hardware::GraphicsPipeline graphics_pipeline, RenderPrimitiveType type)
	{
		if (graphics_pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::setCullMode(hardware::GraphicsPipeline graphics_pipeline, CullMode mode)
	{
		if (graphics_pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::setDepthTest(hardware::GraphicsPipeline graphics_pipeline, bool enabled)
	{
		if (graphics_pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::setDepthWrite(hardware::GraphicsPipeline graphics_pipeline, bool enabled)
	{
		if (graphics_pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::setDepthCompareFunc(hardware::GraphicsPipeline graphics_pipeline, DepthCompareFunc func)
	{
		if (graphics_pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::setBlending(hardware::GraphicsPipeline graphics_pipeline, bool enabled)
	{
		if (graphics_pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::setBlendFactors(hardware::GraphicsPipeline graphics_pipeline, BlendFactor src_factor, BlendFactor dst_factor)
	{
		if (graphics_pipeline == SCAPES_NULL_HANDLE)
			return;
	...
	/*
	 */
	bool Device::resetCommandBuffer(hardware::CommandBuffer command_buffer)
	{
		if (command_buffer == SCAPES_NULL_HANDLE)
			return false;
	...
	bool Device::beginCommandBuffer(hardware::CommandBuffer command_buffer)
	{
		if (command_buffer == SCAPES_NULL_HANDLE)
			return false;
	...
	bool Device::endCommandBuffer(hardware::CommandBuffer command_buffer)
	{
		if (command_buffer == SCAPES_NULL_HANDLE)
			return false;
	...
	bool Device::submit(hardware::CommandBuffer command_buffer)
	{
		if (command_buffer == SCAPES_NULL_HANDLE)
			return false;
	...
	bool Device::submitSyncked(hardware::CommandBuffer command_buffer, hardware::SwapChain wait_swap_chain)
	{
		if (command_buffer == SCAPES_NULL_HANDLE)
			return false;
	...
	bool Device::submitSyncked(hardware::CommandBuffer command_buffer, uint32_t num_wait_command_buffers, const hardware::CommandBuffer *wait_command_buffers)
	{
		if (command_buffer == SCAPES_NULL_HANDLE)
			return false;
	...
	void Device::beginRenderPass(hardware::CommandBuffer command_buffer, hardware::RenderPass render_pass, hardware::FrameBuffer frame_buffer)
	{
	...
	void Device::beginRenderPass(hardware::CommandBuffer command_buffer, hardware::RenderPass render_pass, hardware::SwapChain swap_chain)
	{
		assert(command_buffer != SCAPES_NULL_HANDLE);
		assert(render_pass != SCAPES_NULL_HANDLE);
	...
	void Device::endRenderPass(hardware::CommandBuffer command_buffer)
	{
		if (command_buffer == SCAPES_NULL_HANDLE)
			return;
	...
	void Device::drawIndexedPrimitiveInstanced(
		...
		hardware::GraphicsPipeline graphics_pipeline,
